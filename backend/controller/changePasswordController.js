...
async function resetPassword(request, response, err, user) {
    if (err || !user) {
        logger.log("error", errHandler.userNotFoundErrorHandler(err))
...
const AdminForgotPassword = async(request, response) => {
    ...
    if (user === null) {
        logger.log("error", errHandler.emailNotFoundErrorHandler(error))
    ...
    transporter.sendMail(mailOptions, function(err, info) {
        if (err) {
            logger.log("error", errHandler.errorHandler(error))
...
const AdminResetPassword = async(request, response) => {
    if (request.session.resetLink) {
        jwt.verify(request.session.resetLink, process.env.TOKEN_SECRET, function(err, res) {
            if (err) {
                logger.log("error", errHandler.invalidTokenErrorHandler(error))
        ...
        logger.log("error", errHandler.tokenNotFoundErrorHandler(error))
