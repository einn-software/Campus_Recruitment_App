...
const InstructionAdd = (async(req, res) => {
    if (req.session.user_type == Constants.admin) {
        ...
        if (instructionExist) {
            logger.log('error', `Function Instructions.findOne({code: ${req.body.code}})`, errHandler.codeExistErrorHandler());
...
const InstructionGet = (function(req, res) {
    if (req.session.user_type == Constants.admin) {
        Instructions.find({}, (err, results) => {
            if (err || !results) {
                logger.log("error", "Function Instructions.find({})", errHandler.errorHandler(err))
                return res
...
const InstructionGetById = (function(req, res) {
    if (req.session.user_type == Constants.admin || Constants.student) {
        Instructions.findOne({
            ...
            (err, results) => {
                if (err || !results) {
                    logger.log("error", `Function Instructions.findOne({_id: ${req.params.id}})`, errHandler.idNotFoundErrorHandler());
        ...
        if (req.session.user_type == Constants.admin) {
            Instructions.findOneAndUpdate({
                    ...
                    body, {
                        ...
                        if (err) {
                            logger.log("error", `Function Instructions.findOneAndUpdate({_id: ${req.params.id}, with body ${body}})`, errHandler.errorHandler(err));
                            return res.status(Constants.er_failure).json(errHandler.errorHandler(err));
                        }
                        if (!result) {
                            logger.log("error", `Function Instructions.findOneAndUpdate({_id: ${req.params.id}}), with body ${body}})`, errHandler.idNotFoundErrorHandler());
                            return res
        ...
        if (req.session.user_type == Constants.admin) {
            Instructions.deleteMany({},
                (err, results) => {
                    ...
                    if (!results) {
                        logger.log("error", `Function Instructions.findOneAndUpdate({_id: ${req.params.id}})`, errHandler.idNotFoundErrorHandler(err))
                        return res
                            .status(Constants.er_not_found)
                            .json(errHandler.idNotFoundErrorHandler());
                    }
