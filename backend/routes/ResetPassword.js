        ...
        if (error) {
          ...
          transporter.sendMail(mailOptions, function (error, info) {
            if (error) {
              ...
              response.json({
                Message:
                  "Recovery email sent, Please check your email inbox and spam folder",
                Status_code: 200,
              });
            }
          });
...
router.post("/admin/reset_password", async (request, response) => {
...
/* Reset a password using */
router.post("/student/reset_password", async (request, response) => {
  const { resetLink, newPassword } = request.body;
  if (resetLink) {
    jwt.verify(resetLink, process.env.TOKEN_SECRET, function (err, res) {
      if (err) {
      ...
      (error, success) => {
        if (error) {
          ...
          transporter.sendMail(mailOptions, function (error, info) {
            if (error) {
              console.log(error);
              return res.json(error, { Status_code: 400 }).status(400);
            } else {
              console.log("Email sent: " + info.response);
              response.json({
                Message:
...
/* Reset a password using */
router.post("/college/reset_password", async (request, response) => {
  const { resetLink, newPassword } = request.body;
  if (resetLink) {
    jwt.verify(resetLink, process.env.TOKEN_SECRET, function (err, res) {
      if (err) {
...
/* Reset a password using */
router.post("/tpo/reset_password", async (request, response) => {
  const { resetLink, newPassword } = request.body;
  if (resetLink) {
    jwt.verify(resetLink, process.env.TOKEN_SECRET, function (err, res) {
      if (err) {
